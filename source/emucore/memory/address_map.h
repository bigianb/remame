...
public:
	...
	std::uint8_t        m_bits;             // width of the handler in bits, or 0 for default
	std::uint64_t       m_mask;             // mask for which lanes apply
...
public:
	...
	// unit mask setting
	address_map_entry &umask16(std::uint16_t _mask);
	address_map_entry &umask32(std::uint32_t _mask);
	address_map_entry &umask64(std::uint64_t _mask);
