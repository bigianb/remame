...
typedef std::int64_t attoseconds_t;
typedef std::int32_t seconds_t;
...
public:
	...
	std::uint64_t as_ticks(std::uint32_t frequency) const;
	...
	static attotime from_ticks(std::uint64_t ticks, std::uint32_t frequency);
	/** Create an attotime from a integer count of seconds @seconds */
	static constexpr attotime from_seconds(std::int32_t seconds) { return attotime(seconds, 0); }
	/** Create an attotime from a integer count of milliseconds @msec */
	static constexpr attotime from_msec(std::int64_t msec) { return attotime(msec / 1000, (msec % 1000) * (ATTOSECONDS_PER_SECOND / 1000)); }
	/** Create an attotime from a integer count of microseconds @usec */
	static constexpr attotime from_usec(std::int64_t usec) { return attotime(usec / 1000000, (usec % 1000000) * (ATTOSECONDS_PER_SECOND / 1000000)); }
	/** Create an attotime from a integer count of nanoseconds @nsec */
	static constexpr attotime from_nsec(std::int64_t nsec) { return attotime(nsec / 1000000000, (nsec % 1000000000) * (ATTOSECONDS_PER_SECOND / 1000000000)); }
	...
	attotime &operator-=(const attotime &right);
	attotime &operator*=(std::uint32_t factor);
	attotime &operator/=(std::uint32_t factor);
...
inline attotime operator*(const attotime &left, std::uint32_t factor)
{
...
inline attotime operator*(std::uint32_t factor, const attotime &right)
{
...
inline std::uint64_t attotime::as_ticks(std::uint32_t frequency) const
{
	std::uint32_t fracticks = (attotime(0, m_attoseconds) * frequency).m_seconds;
...
inline attotime attotime::from_ticks(std::uint64_t ticks, std::uint32_t frequency)
{
	...
	std::int32_t secs = divu_64x32_rem(ticks, frequency, &remainder);
	return attotime(secs, std::uint64_t(remainder) * attos_per_tick);
}
